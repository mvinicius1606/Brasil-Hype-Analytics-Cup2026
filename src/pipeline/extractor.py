...
class BrazilHypeWorldCup:
    def __init__(self):
        ...
        self.catalogo_entidades = [
            {"id": 1, "nome": "Alex Sandro", "funcao": "Defensor", "termos": ["alex sandro", "alexsandro", "alexsandro lateral"]},
            {"id": 2, "nome": "Alisson", "funcao": "Goleiro", "termos": ["alisson becker", "alisson"]},
            {"id": 3, "nome": "Bento", "funcao": "Goleiro", "termos": ["bento"]},
            {"id": 4, "nome": "Brasil", "funcao": "Seleção", "termos": ["seleção brasileira", "seleção do brasil", "brasil", "brazil", "canarinho"]},
            ...
            {"id": 12, "nome": "Douglas Santos", "funcao": "Defensor", "termos": ["douglas santos", "douglas"]},
            {"id": 13, "nome": "Ederson (Goleiro)", "funcao": "Goleiro", "termos": ["ederson goleiro", "ederson do fenerbahce", "ederson fenerbahce"]},
            {"id": 14, "nome": "Éderson (Atalanta)", "funcao": "Meio-campista", "termos": ["ederson da atalanta", "ederson atalanta", "ederson volante"]},
            {"id": 15, "nome": "Endrick", "funcao": "Atacante", "termos": ["endrick"]},
            {"id": 16, "nome": "Fabinho", "funcao": "Meio-campista", "termos": ["fabinho"]},
            {"id": 17, "nome": "Gabriel Magalhães", "funcao": "Defensor", "termos": ["gabriel magalhães", "gabriel magalhaes", "magalhães", "magalhaes"]},
            {"id": 18, "nome": "Gabriel Martinelli", "funcao": "Atacante", "termos": ["martinelli", "gabriel martinelli"]},
            {"id": 19, "nome": "Ibañez", "funcao": "Defensor", "termos": ["ibañez", "ibanez"]},
            {"id": 20, "nome": "Igor Jesus", "funcao": "Atacante", "termos": ["igor jesus"]},
            {"id": 21, "nome": "Igor Thiago", "funcao": "Atacante", "termos": ["igor thiago", "thiago brentford"]},
            {"id": 22, "nome": "Léo Pereira", "funcao": "Defensor", "termos": ["léo pereira", "leo pereira"]},
            {"id": 23, "nome": "Lucas Paquetá", "funcao": "Meio-campista", "termos": ["lucas paquetá", "lucas paqueta", "paquetá", "paqueta"]},
            {"id": 24, "nome": "Luiz Henrique", "funcao": "Atacante", "termos": ["luiz henrique", "lh"]},
            {"id": 25, "nome": "Marquinhos", "funcao": "Defensor", "termos": ["marquinhos"]},
            {"id": 26, "nome": "Matheus Cunha", "funcao": "Atacante", "termos": ["matheus cunha", "cunha"]},
            {"id": 27, "nome": "Neymar", "funcao": "Atacante", "termos": ["neymar jr", "neymar júnior", "neymar", "ney", "adulto ney", "menino ney"]},
            {"id": 28, "nome": "Raphinha", "funcao": "Atacante", "termos": ["raphinha"]},
            {"id": 29, "nome": "Rayan", "funcao": "Atacante", "termos": ["rayan"]},
            {"id": 30, "nome": "Vinicius Júnior", "funcao": "Atacante", "termos": ["vinicius jr", "vini jr", "vinicius junior", "vinicius júnior", "vini"]},
            {"id": 31, "nome": "Weverton", "funcao": "Goleiro", "termos": ["weverton"]}
        ]
    ...
    def processar_pipeline_copa(self, data_inicio=None, data_fim=None, meta_posts_por_entidade=100):
        ...
        # Despacho o arquivo consolidado direto para o AWS S3 e atualizo o arquivo local único
        if not df_obt.empty:
            # 1. Envia o histórico particionado para a nuvem (AWS S3)
            self.enviar_dados_s3(df_obt)

            # 2. Cria a pasta 'data' localmente se ela não existir
            pasta_local = "data"
            if not os.path.exists(pasta_local):
                os.makedirs(pasta_local)

            # 3. Salva SEMPRE por cima do mesmo arquivo (sem acumular dados diários no PC)
            caminho_local = os.path.join(pasta_local, "obt_copa_atual.parquet")

            try:
                df_obt.to_parquet(caminho_local, index=False, engine="pyarrow")
                logger.info(f"💾 [LOCAL] Arquivo de trabalho atualizado em: {caminho_local}")
            except Exception as e:
